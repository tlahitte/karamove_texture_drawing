...
def create_material_for_object(obj):
    ...
    default_principled.name = 'Default Principled'
    ...
    if default_texture_name != 'None' and default_texture_name in bpy.data.images:
        ...
        default_tex_node.name = 'Default Texture'
    ...
    if alpha_texture_name != 'None' and alpha_texture_name in bpy.data.images:
        ...
        alpha_tex_node.name = 'Alpha Texture'
        # Connect alpha texture to both shaders
        links.new(alpha_tex_node.outputs['Alpha'], default_principled.inputs['Alpha'])
        links.new(alpha_tex_node.outputs['Alpha'], imported_principled.inputs['Alpha'])
    else:
...
def create_or_update_material(obj, image_path):
    ...
    nodes = mat.node_tree.nodes
    imported_tex_node = nodes.get('Imported Texture')
    ...
    # Update object state to reflect that we are now using the imported texture
    if "object_states" not in addon_data:
        addon_data["object_states"] = {}
    object_state = addon_data["object_states"].get(obj.name, {})
    object_state["use_default_texture"] = False
    addon_data["object_states"][obj.name] = object_state
    save_addon_data()
...
class KaramoveTexturePanelJSON(bpy.types.Panel):
    """
    Panel for JSON operations like loading, saving, and opening the JSON file.
    """
    bl_label = "JSON"
    bl_idname = "OBJECT_PT_karamove_texture_json"
    bl_space_type = 'VIEW_3D'
    bl_region_type = 'UI'
    bl_category = 'Karamove Texture'
    ...
    def draw(self, context):
        ...
        # Load JSON Button
        row = layout.row()
        row.operator("object.karamove_load_json", text="Load JSON")

        # Save JSON Button
        row = layout.row()
        row.operator("object.karamove_save_json", text="Save JSON")

        # Open JSON Location Button
        row = layout.row()
        row.operator("object.karamove_open_json_location", text="Open JSON Location")

class KaramoveLoadJSONOperator(bpy.types.Operator):
    """
    Operator to load the current state of the JSON file and update the panels in Blender.
    """
    bl_idname = "object.karamove_load_json"
    bl_label = "Load JSON"

    def execute(self, context):
        load_addon_data()
        # Force UI to update if necessary
        for window in bpy.context.window_manager.windows:
            for area in window.screen.areas:
                if area.type == 'VIEW_3D':
                    area.tag_redraw()
        self.report({'INFO'}, "Addon data loaded from JSON.")
        return {'FINISHED'}

class KaramoveSaveJSONOperator(bpy.types.Operator):
    """
    Operator to save the current Blender panel states to the JSON file.
    """
    bl_idname = "object.karamove_save_json"
    bl_label = "Save JSON"

    def execute(self, context):
        save_addon_data()
        self.report({'INFO'}, "Addon data saved to JSON.")
        return {'FINISHED'}

class KaramoveOpenJSONLocationOperator(bpy.types.Operator):
    """
    Operator to open the file explorer at the location of the JSON file.
    """
    bl_idname = "object.karamove_open_json_location"
    bl_label = "Open JSON Location"
    ...
    def execute(self, context):
        ...
        if json_file_path and os.path.exists(json_file_path):
            ...
            self.report({'INFO'}, "Opened JSON file location.")
        else:
            self.report({'WARNING'}, "JSON file not found.")
        return {'FINISHED'}
...
def register():
    ...
    bpy.utils.register_class(KaramoveTexturePanelJSON)  # Register the new JSON panel
    ...
    bpy.utils.register_class(KaramoveLoadJSONOperator)
    bpy.utils.register_class(KaramoveSaveJSONOperator)
    bpy.utils.register_class(KaramoveOpenJSONLocationOperator)
...
def unregister():
    ...
    bpy.utils.unregister_class(KaramoveTexturePanelJSON)  # Unregister the JSON panel
    ...
    bpy.utils.unregister_class(KaramoveLoadJSONOperator)
    bpy.utils.unregister_class(KaramoveSaveJSONOperator)
    bpy.utils.unregister_class(KaramoveOpenJSONLocationOperator)
